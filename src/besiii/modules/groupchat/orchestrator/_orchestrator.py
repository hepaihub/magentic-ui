...
from besiii.magentic_utils.types import HumanInputFormat, Plan
from besiii.magentic_utils.magentic_utils import dict_to_str, thread_to_context
from besiii.modules.tools.magentic_tools.bing_search import get_bing_search_results
...
class Orchestrator(BaseGroupChatManager):
    ...
    def __init__(
        ...
        self._model_client_host: ChatCompletionClient = model_client_host
        self._model_client_parser: ChatCompletionClient = model_client_parser
        self._model_client = model_client_parser
        self._model_context = TokenLimitedChatCompletionContext(
            model_client_host, token_limit=config.model_context_token_limit
        )
    ...
    async def _request_next_speaker(
        ...
        if next_speaker == "no_action_agent":
            ...
            return
        # NOTE: 发送给Tester后需要Tester在metadata内添加是否为长任务。
    ...
    async def _orchestrate_step_planning(
        ...
        if self._state.task == "" and self._state.plan_str == "":
            ...
            # create a first plan, XXX: 这里与system meassage 重复
            context.append(
                UserMessage(
